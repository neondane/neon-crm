...
function pageHtml(o) {
  ...
  var reviews = (o.reviews && o.reviews.length) ? o.reviews : [
    ...
    { text: 'They helped us through one of the toughest times of our lives. Our house flooded and we had around 6 tons of debris — the crew hauled it all off and brought some sense of peace back to our lives. Quick, efficient, and I highly recommend them.', by: 'Dylan A.' },
    { text: 'These guys were recommended to move a hot tub I was getting rid of. They showed up on time, handled a challenging removal quickly, and got it delivered right where it needed to go. Great job!', by: 'C.R.' }
  ];
  ...
  // Reveal-on-scroll using element.animate(): it ALWAYS resolves to the natural (visible)
  // state and never pre-hides via persistent CSS, so the page can never get stuck blank.
  var animScript = "<script>(function(){var r=document.getElementById('ngLanding');if(!r||typeof r.animate!=='function'||!('IntersectionObserver' in window))return;var secs=[].slice.call(r.children);function rev(s,i){try{var dx=(i%2?70:-70);s.animate([{opacity:0,transform:'translate('+dx+'px,42px)'},{opacity:1,transform:'none'}],{duration:1300,easing:'cubic-bezier(.16,.68,.28,1)',fill:'backwards'});}catch(e){}}var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){var i=secs.indexOf(e.target);rev(e.target,i<0?0:i);io.unobserve(e.target);}});},{threshold:.04,rootMargin:'0px 0px -50px 0px'});secs.forEach(function(s){io.observe(s);});})();</script>";
  ...
  // Brokerage "trusted by" band — real brokerages Neon Giant works with, written out for SEO.
  var brokers = ['John L. Scott', 'RE/MAX', 'Windermere', 'Coldwell Banker', 'Compass', 'Keller Williams', 'Skagit Tradition Realty', 'Brown McMillen', 'Muljat Group', 'NextHome', 'Century 21', 'eXp Realty'];
  var brokerChips = brokers.map(function (b) {
    return '<span class="ngchip" style="display:inline-block;font-family:' + HEAD + ';font-weight:500;font-size:14px;color:' + WHITE + ';background:' + PANEL2 + ';border:1px solid ' + LINE + ';padding:10px 18px;border-radius:50px;margin:6px">' + esc(b) + '</span>';
  }).join('');
  var brokerBand = '<div style="border-top:1px solid ' + LINE + ';border-bottom:1px solid ' + LINE + ';background:linear-gradient(90deg,rgba(255,47,160,.06),rgba(43,198,255,.06));padding:50px 28px;text-align:center"><div style="max-width:1000px;margin:0 auto"><div style="' + kicker + '">Trusted across the county</div><h2 style="' + h2 + ';font-size:24px">Neon Giant moves clients for top agents at</h2><div style="margin-top:20px">' + brokerChips + '</div><p style="color:' + DIM + ';font-size:13px;margin-top:18px">Proudly serving REALTORS across Skagit &amp; Whatcom County, Washington &mdash; including agents at John L. Scott, RE/MAX, Windermere, Coldwell Banker, Compass, Keller Williams, and more.</p></div></div>';
  var quoteCards = reviews.map(function (r) {
    return '<div class="ngc" style="' + card + ';width:330px;flex:none;white-space:normal"><div style="color:#FFB02E;font-size:15px;letter-spacing:2px">&#9733;&#9733;&#9733;&#9733;&#9733;</div><p style="font-size:15px;color:' + WHITE + ';line-height:1.6;margin:10px 0 0">' + esc(r.text) + '</p><div style="margin-top:14px;font-family:' + HEAD + ';font-weight:500;color:' + DIM + ';font-size:13px">' + esc(r.by) + ' &middot; 5-star Google review</div></div>';
  }).join('');
