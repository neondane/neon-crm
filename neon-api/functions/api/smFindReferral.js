...
const handler = endpoint(async (ctx) => {
  ...
  for (var page = 1; page <= pages; page++) {
    ...
    list.forEach(function (o) {
      ...
      if (q && norm(aff).indexOf(q) >= 0) {
        matches.push({
          id: o.id,
          customer: (o.customer && (o.customer.name || o.customer.fullName)) || o.customerName || '',
          affiliate: aff,
          serviceDate: o.serviceDate || o.moveDate || null,
          status: o.status || o.leadStatus || null,
  ...
  return reply({ ok: true, q: body.q || '', scanned: scanned, matchCount: matches.length, matches: matches, sampleKeys: sampleKeys, lastError: lastError });
});
